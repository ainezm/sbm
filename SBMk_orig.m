...
A = sparse(reord(ai(edges1)),reord(aj(edges1)),ones(size(edges1)),k*n,k*n);
B = sparse(reord(ai(edges2)),reord(aj(edges2)),ones(size(edges2)),k*n,k*n);
...
l=k; %3*k;
colsY2 = Y2([ceil(n/8),ceil(3*n/8),ceil(5*n/8)]);  %(Y2(randperm(length(Y2),l));
...
outC1 = union(outC1,union(bad21,bad31));
    
outC2 = setdiff(outC2,union(bad21,bad23));
outC2 = union(outC2,union(bad12,bad32));
